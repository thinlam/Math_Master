...
import { ACCOUNT, ENDPOINTS, PROFILE_PATH } from '@/constants/auth/reset';
...
export default function ResetPasswordScreen() {
  ...
  const goProfile = () => {
    const href = `${PROFILE_PATH}?reset=1`; // redirect đến trang Profile sau khi reset xong
    try { router.replace(href as any); } catch {}
    if (Platform.OS === 'web') setTimeout(() => { window.location.assign(href); }, 50); // đảm bảo chuyển trang trên web
  };
