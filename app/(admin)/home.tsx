    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#0b1220' }}>
            ...
            >
                ...
                >
                    <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700', marginBottom: 8 }}>Tác vụ nhanh</Text>
                ...
                <Section title="Người dùng mới">
                    <RecentList
                        ...
                        onPressItem={(it) => router.push({ pathname: '/(admin)/users', params: { highlight: it.id } })}
                    />
                </Section>

                <Section title="Bài học mới">
                    <RecentList
                        data={recentLessons}
                        empty="Chưa có dữ liệu."
                        onPressItem={(it) => router.push({ pathname: '/(admin)/lessons', params: { highlight: it.id } })}
                    />
                </Section>

                <Section title="Báo cáo gần đây">
                    <RecentList
                        data={recentReports}
                        empty="Không có báo cáo."
                        onPressItem={(it) => router.push({ pathname: '/(admin)/reports', params: { highlight: it.id } })}
                    />
                </Section>
            </ScrollView>
...
/* ---------- Sub Components ---------- */
function go(router: ReturnType<typeof useRouter>, dest: 'users' | 'lessons' | 'reports' | 'settings' | 'announcements' | 'analytics' | 'admin-config') {
    switch (dest) {
...
function RecentList({ data, empty, onPressItem }: { data: RecentItem[]; empty: string; onPressItem: (it: RecentItem) => void; }) {
    if (!data.length) {
        return <Text style={{ color: '#94a3b8' }}>{empty}</Text>;
    }
    return (
        <FlatList
            data={data}
